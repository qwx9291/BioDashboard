...
public class StringUtil {
	...
	public static Map<String, String> getQueryStringMap(HttpServletRequest request) {
		...
		for (int i = 0; i < parameters.length; i++) {
			int j = parameters[i].indexOf('=');
			if (j > -1)
				rtnVal.put(parameters[i].substring(0, j), parameters[i].substring(j + 1));
		}
		return rtnVal;
	}
...
//			urlPrefix = ResourceUtil.getWebConfigString("web.urlPrefix");
//			if (!isNeedDNS) {
