...
public class LastWeekReportStatisticsJob implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		for (UserDO userDO : users) {
			...
			HashMap<String, ReportCountDO> reportCountDOsMap = new HashMap<String, ReportCountDO>();
			...
			if (null != roleDOs && roleDOs .equals(2)) { // PI,只针对 PI 发邮件
				...
				String title = sdf.format(fromDate) + "-" + sdf.format(toDate) + "-"
						+ deptService.selectById(deptId).getName() + " 日报";
				...
				ThreadPool.getThreadPool().addRunnable(new ThreadPool.Runnable() {
					...
					public void run() {
						try {
							...
							mailBean.setSubject("【BioDashboard】上周的日报统计已经为您生成！");
