...
//	@RequiresPermissions("bio:report:latest")
    ...
    @GetMapping("/list")
    @RequiresPermissions("bio:report:report")
    public Result<Page<ReportDO>> list(Integer pageNumber, Integer pageSize, HttpServletRequest request) throws Exception {
...
//        model.addAttribute("currentUserId",userDO.getId());
        ...
        Wrapper<ReportDO> wrapper = new EntityWrapper<ReportDO>();
...
//		}
    ...
    @RequiresPermissions("bio:report:add")
    @Transactional
    public Result<String> save(HttpServletRequest request, ReportDO report) {
        String fileUrl = request.getParameter("fileHiddenUrl");
        Map<String, Object> param = new HashMap<>();
        param.put("fileUrl", fileUrl);
        param.put("report", report);

        if (reportService.saveReportandFile(param)) {
            return Result.ok();
        }
        return Result.fail();
    }
    ...
    public Result<String> remove(@RequestParam("ids[]") String[] ids) {
        reportService.removeAll(Arrays.asList(ids));
        return Result.ok();
    }
