...
public class RoleServiceImpl extends CoreServiceImpl<RoleDao, RoleDO> implements RoleService {
	...
	@Override
	public String findRoleIdByUserId(Serializable id) {
	    String rolesIds = userRoleMapper.findRoleId(id);
	    return rolesIds;
	}
	
    @Override
    public List<RoleDO> findListByUserId(Serializable userId) {
        String rolesId = userRoleMapper.findRoleId(userId);
        List<RoleDO> roles = selectList(null);
...
//            }
    ...
    public boolean updateById(RoleDO role) {
        ...
        List<String> menuIds = role.getMenuIds();
        String roleId = role.getId();
        r = roleMenuMapper.removeByRoleId(roleId);
        List<RoleMenuDO> rms = new ArrayList<>();
        for (String menuId : menuIds) {
            RoleMenuDO rmDo = new RoleMenuDO();
            rmDo.setRoleId(roleId);
        ...
        if (rms.size() > 0) {
           r = roleMenuMapper.batchSave(rms);
        }
        return retBool(r);
    }
